...
const PostDetail = ({ route}) => {
  ...
  const date = data.start_time.split(' ')[0], stime = data.start_time.split(' ')[1];
  return (
    <ScrollView>
      <View style={[styles.container,{marginHorizontal:20}]}>
        ...
        <Text style={{ fontSize:20,marginVertical:15}}>日期: {date}</Text>
        <Text style={{ fontSize:20,marginVertical:15}}>時間: {stime}</Text>
